...
exports.updateAGM = async (req, res) => {
  try {
    const [updated] = await AGM.update(req.body, { where: { id: req.params.id } });
...
exports.getAGMsByMember = async (req, res) => {
  try {
    ...
    if (usertypeid === 1) {
      ...
      const member = await require("../models").Member.findOne({ where: { userid: id } });
      if (!member) return res.status(404).json({ error: "Member not found" });
      const memberRoles = await require("../models").MemberRole.findAll({ where: { member_id: member.id, status: 'active' } });
