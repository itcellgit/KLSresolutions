...
// *** NOW USING API_BASE_URL DIRECTLY - Single source of truth ***
...
export const getAllMemberRoles = async (token) => {
  try {
    const response = await axios.get(`${API_BASE_URL}/memberrole`, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
...
export const deleteMemberRole = async (id, token) => {
  try {
    const response = await axios.delete(`${API_BASE_URL}/memberrole/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
...
export const getMemberRoles = async (member_id, token) => {
  try {
    const response = await axios.get(
      `${API_BASE_URL}/members/${member_id}/roles`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
