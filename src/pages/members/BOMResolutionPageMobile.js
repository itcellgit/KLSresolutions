...
import { searchPDFContent } from "../../api/bomResolutions";

const { width } = Dimensions.get("window");

const BOMResolutionPageMobile = () => {
...
`;
  ...
  // Search across PDF content on the server
  const performPdfSearch = async (searchText) => {
    ...
    try {
      ...
      const arr = results?.results ?? (Array.isArray(results) ? results : []);
  ...
  // Debounced effect for pdfSearchTerm
  useEffect(() => {
    const id = setTimeout(() => {
      if (pdfSearchTerm && pdfSearchTerm.trim()) {
        performPdfSearch(pdfSearchTerm.trim());
      } else {
        setSearchResults([]);
      }
    }, 500);
  ...
  const getBOMResolutions = async (token) => {
    const url =
      `${API_BASE_URL}/bom_resolutions` +
      (selectedTenure
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const bomData = await getBOMResolutions(token);
      const resolutions =
        bomData?.resolutions ?? (Array.isArray(bomData) ? bomData : []);
  ...
  const filteredData = (bomResolutions || []).filter((item) => {
    ...
    const matchesSearch =
      ...
      String(item.meeting_date || item.bom_date || "")
        .toLowerCase()
        .includes(searchLower);
  ...
  // Use search results when a global PDF search is active, otherwise use filteredData
  const dataToGroup = pdfSearchTerm.trim() ? searchResults : filteredData;

  const groupedByDate = (dataToGroup || []).reduce((acc, item) => {
    const dateKey = item.bom_date || item.meeting_date || "N/A";
  ...
  const handlePDFView = async (type, filename) => {
    ...
    const remote = `${API_BASE_URL}/bom_resolutions/file/${encodeURIComponent(
      filename
    )}`;
    ...
    try {
      ...
      if (headers && headers.Authorization) {
        ...
        const html = buildPdfJsHtmlFromUrl(remote, token);
        setPdfHtml(html);
        setPdfRemoteUri("");
      } else {
        // For public PDFs without auth
        setPdfHtml("");
        setPdfRemoteUri(remote);
      }
  ...
  const handleTabClick = async (tab) => {
    ...
    const fileExistsKey = `${tab}_exists`;
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading BOM Resolutions...</Text>
        </View>
      </SafeAreaView>
    );
  }
        ...
        >
          <View style={styles.headerSection}>
            ...
            <Text style={styles.pageSubtitle}>
              View and search BOM resolutions
            </Text>
          </View>

          {(filteredInstitutes.length > 0 || tenures.length > 0) && (
                ...
                {tenures.length > 0 && (
                  <View style={styles.tenureContainer}>
                    ...
                    <View style={styles.tenureDropdownWrapper}>
                      ...
                      >
                        <Text
                          style={styles.tenureDropdownText}
                          numberOfLines={1}
                        >
                          {selectedTenure
                            ? tenures.find(
                  ...
                  <View key={monthYearKey} style={styles.monthContainer}>
                    ...
                    {hasSelectedDate && selectedDate && (
                      <View style={styles.expandedContent}>
                        ...
                        {/* PDF Viewer */}
                        {viewingPDF && (pdfHtml || pdfRemoteUri) && (
...
export default BOMResolutionPageMobile;
