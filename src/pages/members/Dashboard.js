          ...
          {cardData.map((card) => (
            ...
            >
              <div className="flex flex-col items-center justify-center h-full">
                <span className="mb-6 text-6xl md:text-7xl animate-bounce-slow" aria-label={card.title}>
                  {card.icon}
                </span>
                <h2 className={`mb-2 text-2xl md:text-3xl font-bold ${card.text} group-hover:text-white text-center transition-colors font-serif`}>
                  {card.title}
                </h2>
