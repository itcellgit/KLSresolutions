...
const BOMResolutionsPage = () => {
  ...
  const filteredResolutions = resolutions.filter((resolution) => {
    const gcResolution = gcResolutions.find(
      (gc) => gc.id === resolution.gc_resolution_id
    );
    return (
                  ...
                  filteredResolutions.map((resolution) => (
                    <tr key={resolution.id}>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500 max-w-xs">
                        <div
                          className="truncate"
                          title={getGCResolutionDetails(
                            resolution.gc_resolution_id
                          )}
                        >
                          {getGCResolutionDetails(resolution.gc_resolution_id)}
                        </div>
                      </td>
                      ...
                      <div>
                        ...
                        >
                          ...
                          {gcResolutions.map((gcResolution) => (
                            ...
                            >
                              ID: {gcResolution.id} -{" "}
                              {gcResolution.agenda.substring(0, 50)}...
                            </option>
                          ))}
                        </select>
                      </div>
