...
//   // Fetch tenures on component mount
//   useEffect(() => {
//     if (token) {
//       fetchTenures();
//     }
...
//   // Reset states when tenure changes
//   useEffect(() => {
...
//   }, [pdfSearchTerm, resolutions, tenures]);
...
//           setViewingPDF(null);
//           setPdfUrl("");
//           setFileError(`Failed to load ${type.replace("-", " ")} file.`);
//         }
//       } catch (error) {
//         console.error("Error fetching PDF:", error);
//         setViewingPDF(null);
//         setPdfUrl("");
//         setFileError(`Error loading ${type.replace("-", " ")} file.`);
//       }
//     } else {
//       setViewingPDF(null);
//       setPdfUrl("");
...
//     switch (tab) {
//       case "agenda":
//         filename = currentData?.agenda_file || currentData?.agenda;
//         break;
//       case "resolution":
//         filename = currentData?.resolution_file || currentData?.resolution;
//         break;
//       case "compliance":
//         filename = currentData?.compliance_file || currentData?.compliance;
//         break;
//       default:
//         setActiveTab(tab);
//         setViewingPDF(null);
...
//   // Filter resolutions based on tenure
//   const filteredData = resolutions.filter((resolution) => {
...
//                                                     PDF viewing in Safari
//                                                     requires opening in a new
//                                                     tab
//                                                   </p>
...
const BOMResolution = () => {
  ...
  // PDF Search functionality - Fixed version
  const performPdfSearch = async (searchText) => {
    if (!searchText.trim()) {
      setSearchResults([]);
      setFileError("");
    ...
    if (!token) {
      ...
      setFileError("Authentication required for search");
      return;
    }
    ...
    try {
      console.log("Component: Starting search for:", searchText);

      const results = await searchPDFContent(searchText, token);
      console.log("Component: Search results received:", results);
      ...
      if (results) {
        ...
        } else if (results.resolutions && Array.isArray(results.resolutions)) {
          searchData = results.resolutions;
        }
      }

      console.log("Component: Processed search data:", searchData);
      setSearchResults(searchData);

      if (searchData.length === 0) {
        console.log("Component: No results found for search term");
      }
    } catch (error) {
      console.error("Component: Error performing PDF search:", error);
      setSearchResults([]);
      setFileError(`Search failed: ${error.message}`);
    } finally {
      setIsSearching(false);
    }
  };
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timeoutId);
  }, [pdfSearchTerm, token]); // Only depend on search term and token
  ...
  // Filter resolutions based on tenure - Using useMemo for performance
  ...
  }, [resolutions, selectedTenure]);

  // Group by date - Updated to handle search results properly
  const dataToGroup = useMemo(() => {
    if (pdfSearchTerm.trim() && searchResults.length > 0) {
      ...
      // If searching but no results found, return empty array
      return [];
    } else {
      // Not searching, use filtered data
      return filteredData;
    }
  }, [pdfSearchTerm, searchResults, filteredData, isSearching]);
  ...
  const groupedByMonthYear = useMemo(() => {
    ...
    Object.keys(groupedByDate).forEach((dateKey) => {
      ...
      grouped[monthYearKey][dateKey] = groupedByDate[dateKey];
    });
    return grouped;
  }, [groupedByDate]);
  ...
  const sortedMonthYearKeys = useMemo(() => {
    return Object.keys(groupedByMonthYear).sort((a, b) => {
      return new Date(b) - new Date(a);
    });
  }, [groupedByMonthYear]);
