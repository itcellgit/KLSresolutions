...
const Sidebar = () => {
  ...
  return (
    <div className="hidden md:flex md:flex-shrink-0">
      <div className="flex flex-col w-72 bg-gradient-to-br from-white via-indigo-50 to-indigo-100 border-r border-gray-200 shadow-xl min-h-screen">
        <div className="flex flex-col items-center justify-center h-32 px-4 bg-white border-b border-indigo-200">
          <img src={process.env.PUBLIC_URL + '/image.png'} alt="KLS Logo" className="w-16 h-16 mb-2 rounded-full shadow-lg" />
          <h1 className="text-2xl font-extrabold text-indigo-700 tracking-wide font-serif mb-1 text-center">KLS Resolutions</h1>
          <span className="text-xs font-medium text-gray-500 text-center">Management Portal</span>
        </div>
            ...
            {navigation.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`group flex items-center px-4 py-3 text-base font-semibold rounded-lg transition-all duration-150 ${
                    location.pathname === item.href
                      ? "bg-indigo-100 text-indigo-700 shadow"
                      : "text-gray-700 hover:bg-indigo-50 hover:text-indigo-700"
                  }`}
                >
                  <div className="w-6 h-6 mr-4 text-indigo-700 group-hover:text-indigo-900">
                    {item.icon}
                  </div>
                  {item.name}
                </Link>
